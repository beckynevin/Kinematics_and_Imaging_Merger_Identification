# Version of MergerMonger for only early stage mergers
...
def run_LDA_simulations(feature_dict, df, name):
...
# Now figure out how many terms you want overall.
    ...
    plt.savefig('determine_n_comps_'+str(input)+'.png', dpi=1000)
...
# This is how to emeasure accuracy, precision, etc...
    ...
    return inputs_all, list_coef[new_min_index][0], list_coef_std[new_min_index], list_inter[new_min_index], list_inter_std[new_min_index], list_std_scale[new_min_index], list_sklearn[new_min_index]
...
df_early = pd.io.parsers.read_csv(filepath_or_buffer='LDA_prep_predictors_all_combined_major_early.txt',header=[0],sep='\t')


early_result = run_LDA_simulations(feature_dict, df_early, 'early')
#return input_all, list_coef[new_min_index], list_coef_std[new_min_index], list_inter[new_min_index], list_inter_std[new_min_index]
print('early result', early_result)
df_late = pd.io.parsers.read_csv(filepath_or_buffer='LDA_prep_predictors_all_combined_major_late.txt',header=[0],sep='\t')


late_result = run_LDA_simulations(feature_dict, df_late, 'late')
print('late result', late_result)
...
# LDA values and stds.
file=open('Inputs_MergerMonger_Novice_early_late.txt','w')
file.write('~~~~Results from Early~~~~'+'\n')
for j in range(len(early_result[0])):
    
    file.write((early_result[0][j])+'\t')
file.write('\n')



for j in range(len(early_result[1])):
    
    file.write(str(early_result[1][j])+'\t')
file.write('\n')

for j in range(len(early_result[2])):

    file.write('std'+str(early_result[2][j])+'\t')
file.write('\n')
file.write(str(early_result[3])+'\n')
file.write('std'+str(early_result[4])+'\n')



print(late_result[1])
file.write('~~~~Results from Late~~~~'+'\n')
for j in range(len(late_result[0])):
    
    file.write((late_result[0][j])+'\t')
file.write('\n')

for j in range(len(late_result[0])):
    file.write(str(late_result[1][j])+'\t')
file.write('\n')



for j in range(len(late_result[2])):
    
    file.write('std'+str(late_result[2][j])+'\t')
file.write('\n')
file.write(str(late_result[3])+'\n')
file.write('std'+str(late_result[4])+'\n')
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
X_list_here=df_early[early_result[0]].values
#and standardize it
std_mean=[float(x) for x in early_result[5].mean_]
std_std=[float(np.sqrt(x)) for x in early_result[5].var_]
X_std_here_early=[]
for j in range(len(X_list_here)):
    X_std_here_early.append(list((X_list_here[j]-std_mean)/std_std))


#print(list_sklearn[new_min_index].predict(X_std_here))
LDA_ID_early=early_result[6].predict(X_std_here_early)

nonmerg_gini_LDA_early=[]
nonmerg_m20_LDA_early=[]
...
LDA_LDA_merg_early=[]
LDA_LDA_nonmerg_early=[]

'''
return inputs_all, list_coef[new_min_index], list_coef_std[new_min_index], list_inter[new_min_index], list_inter_std[new_min_index], list_std_scale[new_min_index], list_sklearn[new_min_index]
'''
coef_early = early_result[1]
inter_early = early_result[3]

# Go through and grab the values of all the predictors for the merging and nonmerging pop
# of simulated galaxies:
for j in range(len(df_early)):
 
    
    if LDA_ID_early[j]==1:#then its a nonmerger
        nonmerg_gini_LDA_early.append(df_early['Gini'].values[j])
        nonmerg_m20_LDA_early.append(df_early['M20'].values[j])
        nonmerg_C_LDA_early.append(df_early['Concentration (C)'].values[j])
        nonmerg_A_LDA_early.append(df_early['Asymmetry (A)'].values[j])
        nonmerg_S_LDA_early.append(df_early['Clumpiness (S)'].values[j])
        nonmerg_n_LDA_early.append(df_early['Sersic N'].values[j])
        nonmerg_A_S_LDA_early.append(df_early['Shape Asymmetry'].values[j])
        LDA_LDA_nonmerg_early.append(np.sum(coef_early*X_std_here_early[j]+inter_early))
        
    if LDA_ID_early[j]==2:#then its a nonmerger
        merg_gini_LDA_early.append(df_early['Gini'].values[j])
        merg_m20_LDA_early.append(df_early['M20'].values[j])
        merg_C_LDA_early.append(df_early['Concentration (C)'].values[j])
        merg_A_LDA_early.append(df_early['Asymmetry (A)'].values[j])
        merg_S_LDA_early.append(df_early['Clumpiness (S)'].values[j])
        merg_n_LDA_early.append(df_early['Sersic N'].values[j])
        merg_A_S_LDA_early.append(df_early['Shape Asymmetry'].values[j])
        LDA_LDA_merg_early.append(np.sum(coef_early*X_std_here_early[j]+inter_early))
...
# It should really be its own function
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
X_list_here=df_late[late_result[0]].values
#and standardize it
std_mean=[float(x) for x in late_result[5].mean_]
std_std=[float(np.sqrt(x)) for x in late_result[5].var_]
X_std_here_late=[]
for j in range(len(X_list_here)):
    X_std_here_late.append(list((X_list_here[j]-std_mean)/std_std))


#print(list_sklearn[new_min_index].predict(X_std_here))
LDA_ID_late=late_result[6].predict(X_std_here_late)

nonmerg_gini_LDA_late=[]
nonmerg_m20_LDA_late=[]
...
LDA_LDA_merg_late=[]
LDA_LDA_nonmerg_late=[]

'''
return inputs_all, list_coef[new_min_index], list_coef_std[new_min_index], list_inter[new_min_index], list_inter_std[new_min_index], list_std_scale[new_min_index], list_sklearn[new_min_index]
'''
coef_late = late_result[1]
inter_late = late_result[3]
...
for j in range(len(df_late)):
    ...
    if LDA_ID_late[j]==1:#then its a nonmerger
        nonmerg_gini_LDA_late.append(df_late['Gini'].values[j])
        nonmerg_m20_LDA_late.append(df_late['M20'].values[j])
        nonmerg_C_LDA_late.append(df_late['Concentration (C)'].values[j])
        nonmerg_A_LDA_late.append(df_late['Asymmetry (A)'].values[j])
        nonmerg_S_LDA_late.append(df_late['Clumpiness (S)'].values[j])
        nonmerg_n_LDA_late.append(df_late['Sersic N'].values[j])
        nonmerg_A_S_LDA_late.append(df_late['Shape Asymmetry'].values[j])
        LDA_LDA_nonmerg_late.append(np.sum(coef_late*X_std_here_late[j]+inter_late))
        
    if LDA_ID_late[j]==2:#then its a nonmerger
        merg_gini_LDA_late.append(df_late['Gini'].values[j])
        merg_m20_LDA_late.append(df_late['M20'].values[j])
        merg_C_LDA_late.append(df_late['Concentration (C)'].values[j])
        merg_A_LDA_late.append(df_late['Asymmetry (A)'].values[j])
        merg_S_LDA_late.append(df_late['Clumpiness (S)'].values[j])
        merg_n_LDA_late.append(df_late['Sersic N'].values[j])
        merg_A_S_LDA_late.append(df_late['Shape Asymmetry'].values[j])
        LDA_LDA_merg_late.append(np.sum(coef_late*X_std_here_late[j]+inter_late))
...
ax1.annotate('Pre-', xy=(0.01,1.12), xycoords='axes fraction', color='orange', size=12)

ax1.annotate('Post-Coalescence', xy=(0.01,1.03), xycoords='axes fraction', color='red', size=12)
...
sns.kdeplot(merg_m20_LDA_early, merg_gini_LDA_early, cmap="Oranges_r", shade=True,shade_lowest=False, alpha=0.7, label='Pre')# bw=.15)
sns.kdeplot(merg_m20_LDA_late, merg_gini_LDA_late, cmap="Reds_r", shade=True,shade_lowest=False, alpha=0.7, label='Post')# bw=.15)
ax1.set_xlim([0,-3])
...
sns.kdeplot(nonmerg_m20_LDA_early, nonmerg_gini_LDA_early, cmap="Blues_r", shade=True,shade_lowest=False)#, alpha=0.5)
...
plt.savefig('gini_m20_contour_LDA_early_late.pdf', bbox_inches='tight')
...
sns.kdeplot(merg_A_LDA_early, merg_C_LDA_early, cmap="Oranges_r", shade=True,shade_lowest=False, alpha=0.7)

sns.kdeplot(merg_A_LDA_late, merg_C_LDA_late, cmap="Reds_r", shade=True,shade_lowest=False, alpha=0.7)
...
ax2.set_title('Nonmergers', loc='right')
plt.axvline(x=0.35, ls='--', color='black')
sns.kdeplot(nonmerg_A_LDA_early, nonmerg_C_LDA_early, cmap="Blues_r", shade=True,shade_lowest=False)
plt.savefig('C_A_contour_LDA_early_late.pdf', bbox_inches='tight')
...
sns.kdeplot(merg_A_S_LDA_early, merg_n_LDA_early, cmap="Oranges_r", shade=True,shade_lowest=False, alpha=0.7)

sns.kdeplot(merg_A_S_LDA_late, merg_n_LDA_late, cmap="Reds_r", shade=True,shade_lowest=False, alpha=0.7)
...
ax2=fig.add_subplot(122)
sns.kdeplot(nonmerg_A_S_LDA_late, nonmerg_n_LDA_late, cmap="Blues_r", shade=True,shade_lowest=False)
...
#          ncol=2)
ax2.set_title('Nonmergers', loc='right')
plt.savefig('n_A_S_contour_LDA_early_late.pdf', bbox_inches='tight')
...
# Perhaps try to make a table that lists the probability of merging
# for both pre- and post-coalescence
# This can eventually expand to include major and minor mergers?
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
X_gal_early = df2[early_result[0]].values
X_gal_late = df2[late_result[0]].values




std_mean_early=[float(x) for x in early_result[5].mean_]
std_std_early=[float(np.sqrt(x)) for x in early_result[5].var_]
std_mean_late=[float(x) for x in late_result[5].mean_]
std_std_late=[float(np.sqrt(x)) for x in late_result[5].var_]
X_std_MaNGA_early=[]
X_std_MaNGA_late=[]
testing_C=[]
testing_A=[]
...
testing_n=[]
testing_A_S=[]
for j in range(len(X_gal_early)):
    X_std_MaNGA_early.append(list((X_gal_early[j]-std_mean_early)/std_std_early))
    X_std_MaNGA_late.append(list((X_gal_late[j]-std_mean_late)/std_std_late))
...
file_prob = open('probability_all.txt', 'w')
file_prob.write('ID'+'\t'+'p_merg_pre'+'\t'+'p_merg_post'+'\n')
if len(X_std_MaNGA_early) != len(X_std_MaNGA_late):

    print('lengths', len(X_std_MaNGA_early), len(X_std_MaNGA_late))
    STOP
for j in range(len(X_gal_early)):
    file_prob.write(str(df2['ID'].values[j])+'\t'+str(early_result[6].predict_proba(X_std_MaNGA_early)[j][1])+'\t'+str(late_result[6].predict_proba(X_std_MaNGA_late)[j][1])+'\n')
